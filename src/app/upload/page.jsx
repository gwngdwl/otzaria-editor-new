...
export default function UploadPage() {
  ...
  const loadRecentUploads = async () => {
    try {
      ...
      if (result.success) {
        setRecentUploads(result.uploads.slice(0, 5))
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        setMessage({ type: 'success', text: `✅ הספר "${bookName}" הועלה בהצלחה!` })
        setFile(null)
        setBookName('')
        document.getElementById('file-input').value = ''
        loadRecentUploads()
      } else {
        setMessage({ type: 'error', text: `❌ ${result.error || 'שגיאה בהעלאת הקובץ'}` })
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-8">
            ...
            <h1 className="text-4xl font-bold mb-2 text-on-surface">שליחת ספר חדש</h1>
            <p className="text-on-surface/70">העלה קובץ טקסט של ספר לספרייה</p>
          </div>

          <div className="glass-strong p-8 rounded-2xl">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-sm font-bold text-on-surface mb-2">שם הספר *</label>
                <input
                  type="text"
                  value={bookName}
              ...
              <div>
                <label className="block text-sm font-bold text-on-surface mb-2">קובץ טקסט *</label>
                <div className="relative">
                  <input id="file-input" type="file" accept=".txt" onChange={handleFileChange} className="hidden" required />
                  <label
                    htmlFor="file-input"
                    ...
                    <div className="text-center">
                      {file ? (
                        <>
                          ...
                          <p className="text-sm text-on-surface/60">{(file.size / 1024).toFixed(2)} KB</p>
                        </>
                      ) : (
                        <>
              ...
              {message && (
                <div className={`p-4 rounded-lg ${message.type === 'success' ? 'bg-green-50 border-2 border-green-200 text-green-800' : 'bg-red-50 border-2 border-red-200 text-red-800'}`}>
              ...
              <div className="flex gap-4">
                ...
                <Link href="/dashboard" className="px-6 py-4 border-2 border-surface-variant text-on-surface rounded-lg hover:bg-surface transition-colors flex items-center justify-center">
                  ביטול
                </Link>
              </div>
            </form>
          ...
          <div className="mt-8 glass p-6 rounded-xl">
            ...
            {loadingUploads ? (
                ...
                {recentUploads.map((upload) => (
                  <div key={upload.id} className="flex items-center gap-4 p-4 bg-surface rounded-lg">
                    ...
                    <div className="flex-1">
                      <p className="font-medium text-on-surface">{upload.book_name}</p>
                      <p className="text-sm text-on-surface/60">
                        {new Date(upload.created_at).toLocaleDateString('he-IL', { day: 'numeric', month: 'long', year: 'numeric' })}
                      </p>
                    </div>
              ...
              <p className="text-sm text-on-surface/60 text-center py-4">עדיין לא העלית ספרים</p>
            )}
          </div>
        </div>
      </div>
